...
Machine::ReadMem(int addr, int size, int *value)
{
    ...
    //在正式开始读数据到物理内存前，需要先调用Translate将虚拟地址翻译为物理地址physicalAddress
...
// from the virtual address
    vpn = (unsigned) virtAddr / PageSize;			//计算虚拟页号
    offset = (unsigned) virtAddr % PageSize;		//页内偏移地址
    
    if (tlb == NULL) {		// => page table => vpn is index into table
	if (vpn >= pageTableSize) {
	    DEBUG(dbgAddr, "Illegal virtual page # " << virtAddr);
	    return AddressErrorException;
	} else if (!pageTable[vpn].valid) {		//valid = false,该页不可用，valid置位，则不再执行虚拟地址到物理地址的转换
	    DEBUG(dbgAddr, "Invalid virtual page # " << virtAddr);
	    return PageFaultException;
	}
	entry = &pageTable[vpn];				//返回虚拟页在页表中对应的页表项
    } else {
    ...
    pageFrame = entry->physicalPage;		//得到物理页号
    ...
    if (writing)
	entry->dirty = TRUE;
    *physAddr = pageFrame * PageSize + offset;		//根据物理页号和页内偏移地址计算实际物理地址
