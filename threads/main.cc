...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    char *userProgName = NULL;        // default is not to execute a user prog 用户程序名称
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    userProgName = argv[i + 1];           //给用户程序名称赋值
	    i++;
	}
	else if (strcmp(argv[i], "-K") == 0) {    //-K表示运行线程测试程序
	    threadTestFlag = TRUE;
	}
    ...
    kernel = new Kernel(argc, argv);		//实例化内核

    kernel->Initialize();					//初始化内核
    ...
    if (userProgName != NULL) {		//若用户程序名不为空，则为用户程序分配空间
      AddrSpace *space = new AddrSpace;		//调用AddrSpace类分配空间
      ASSERT(space != (AddrSpace *)NULL);		//分配完空间再load用户程序
      if (space->Load(userProgName)) {  // load the program into the space	加载用户程序
	space->Execute();              // run the program							执行用户程序
