...
{
    name = threadName;		//设置线程名称
    stackTop = NULL;                   //设置栈顶指针int*
    stack = NULL;                        //设置栈底指针int*
    status = JUST_CREATED;        //设置线程状态
    for (int i = 0; i < MachineStateSize; i++) {    //设置寄存器值
...
//----------------------------------------------------------------------
// Thread::Fork
// 	Invoke (*func)(arg), allowing caller(调用者) and callee(被调用者) to execute 
//	concurrently.
//
...
{
    Interrupt *interrupt = kernel->interrupt;			//中断对象
    Scheduler *scheduler = kernel->scheduler;             //调度对象
    IntStatus oldLevel;
    ...
    oldLevel = interrupt->SetLevel(IntOff);                      //关中断
    scheduler->ReadyToRun(this);	// ReadyToRun assumes that interrupts //将线程插入就绪队列
					// are disabled!
...
Thread::Begin ()
{
    ...
    kernel->scheduler->CheckToBeDestroyed();        //若上一个运行的线程已结束，则销毁它
    kernel->interrupt->Enable();                               //开中断，以得到时间信号
}
...
SimpleThread(int which)
{
    ...
    for (num = 0; num < 5; num++) {
	cout << "*** my  thread " << which << " looped " << num << " times\n";
